...
## P8 / P9 / P10（2026-09-20）

| 编号 | 内容 | 状态 |
|---|---|---|
| P8 | 绿跑判定按**命令段**收紧（ls/cat 不算）+ 收工体检给最终产物的验没验过结论 + 机器自己跑验收脚本 | ✅ ad8c9fb |
| P9-a | 协商闸过半放开时**必须广播**「可以挂片了」（窗口过半 + 已有人广播才放开） | ✅ ad8c9fb |
| P9-b | 墙钟扫（每步）也推进协商闸 —— 修掉「最后没人再试一次、保底晚 100 秒」 | ✅ ad8c9fb |
| P10-a | **挂死红灯**：退出码 124/137/143 当一等公民 —— 当场邮件点名那个 agent、记账、50% 闸广播全队、收工体检写明 | ✅ |
| P10-b | 系统验收闸**不只看退出码**：输出里有「失败/FAIL/❌/Traceback/assert」也算没过（修「只 print 不非零退出」） | ✅ |
| P10-c | **证据作废当场广播**：产物在最后一次验收之后又被改 → 立刻告诉全队「现在这个版本没验过」（每轮最多 3 次） | ✅ |

实测依据（p2482-p9）：三个 agent 都看到了 `退出码 124（30032ms）`，但系统不记账；
agent 的 test_script.py 没有 `timeout=`、失败只 print（退出码永远 0）；betty 在 08:26:31
用 `git show 592fa2b:p2482.cpp > p2482.cpp` 回滚，最终交付的版本根本没验过。
