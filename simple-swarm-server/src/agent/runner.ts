  ...
  private openSkillHandoff(): void {
    try {
      ...
      const lines = [
        ...
        "- 只写可跨任务复用的开发注意点：工具语义、环境与版本、权限与路径、引号/换行/分号、验证和并发协作。",
        "- 不要写具体函数的修改方案、算法答案或测试期望值；过滤器只能辅助检查，不能保证识别所有泄题内容。",
        "- 每条写清适用环境、观察到的现象、证据与建议；猜测标待验证，不要把某个工具的一次失败推广为所有环境都不能用。",
      ];
      ...
      const draft = [
        ...
        "- 收尾前最后一次改动必须早于最后一次验收，否则交出去的证据是过期的。",
      ...
      this.mailTeam(
        ...
        "系统已经把本轮的机器证据落到工作区 EXPERIENCE_RAW.md（分数、退步警报、挂死、最终版本）。\n"
          + "请谁有空把它总结成 EXPERIENCE.md：只写环境、工具和通用开发注意点，每条附适用范围和实际证据，"
          + "别写题面答案（会被机器整行丢掉）。\n"
    ...
    this.mailTeam(
      ...
      "系统现在按现状入账（墙钟 " + String(Math.round(SWARM_AUTOSHIP_FRACTION * 100)) + "%）。\n"
        ...
        + (best.length > 0 && deliv.length > 0
          ...
          : "")
        + "收尾规则：**最后一次改动必须早于最后一次验收** —— 改完自己跑一遍题面验收再收工。",
      "verify",
    );
