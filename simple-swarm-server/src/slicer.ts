...
const SLICER_SYSTEM = [
  ...
  "",
  "★ 第 1 片必须是集成片：片名写成「【前 18 分钟必须交付】把所有产出整合进唯一交付件并跑通验收脚本」，它必须排在切片列表的最前面；所有其余片都排在它后面。没有集成片 = 整个集群交不出成品。",
  "硬规则（违反任何一条都算失败）：",
  "1. **片数严格按人手来**：用户消息里会给出【片数要求】N —— 核心产物必须拆成**正好 N 片**小活，" +
    "既不能把整个产物写成一条切片，也不能只给 6 片（人手多了会有一半 agent 没活干）。",
  "   例：若任务是「一个带动画的单文件作品」，就拆成 造型 / 主要部件 / 部件之间的耦合 / 时序 这样几片；若是算法题，就拆成 数据结构 / 核心算法 / 边界与测试 / 独立对拍 这样几片。",
  "2. **耦合的部件必须留在同一片**：如果两个部件的运动必须互相对齐（腿末端要一直踩在踏板上、",
  "   齿轮要啮合、接口要匹配），它们属于同一片；切开必然对不上。",
  "3. 必须有 **1~2 片是可机械判定的断言**：能在命令行跑出数字、写死阈值（例如「某两点间距 ≤ 5px」「某比值误差 ≤ 1%」「某函数输出等于 62」），",
...
/** 从题面里抠出交付件名（题面通常点名产物：p2482.cpp / main.py / index.html …）。
 *  抠不到就返回空串，保底切法退化成不带文件名的通用说法。 */
export function goalDeliverable(goal: string): string {
  const found = goal.match(/[A-Za-z0-9_-]+\.(cpp|cc|cxx|py|js|mjs|ts|java|go|rs|c|h|html|css|svg|json|md|txt)/);
  return found ? found[0] : "";
}

/** 保底切法：协商立板超时 / LLM 派工失败时用，保证集群**绝不会在空板上开跑**。
 *
 *  2026-09-19 实测（p2482-p3）：旧版这里写死的是动画实验的片（「主体静态造型」
 *  「相邻帧像素差 ≥ 1%」），给一道信息学题架出来 6 片画图片的活 —— **板子完全不贴题**，
 *  而且 5 分钟协商窗口一超时就必然触发。
 *  新版原则：句子对任何任务都成立（读题 / 骨架 / 核心实现 / 边界 / 复检 / 收口），
 *  再从题面抠一个交付件名注进去 —— 通用的同时让人一眼看出这是哪道题的活。 */
export function genericSlices(agents: number, goal = ""): string[] {
  const n = Math.max(6, Math.min(16, Math.round(agents * 1.5)));
  const deliverable = goalDeliverable(goal);
  const art = deliverable ? "「" + deliverable + "」" : "最终交付件";
  const all = [
    "读题 + 把验收标准写死：题面要求的输入 / 输出 / 样例，写成一条能跑的验收命令并跑通（现在是红的也行，但必须打印数字）",
    "【最先交付】交出最小可运行骨架：" + art + " 要在工作目录根，能被题面的样例输入跑起来（输出对不对先不管），并让验收脚本真的执行到它",
    "核心实现：把题面的主要规则 / 主体效果做进 " + art + "，用题面样例实测，把实测输出**原样**贴进 evidence",
    "边界与反例：题面点名的边界（含题面末尾那几条说明）+ 自己造的 2 组以上极端输入，各跑一遍并记录结果",
    "独立复检：另一个人**不看原作者的说法**重新跑一遍 " + art + " 和验收脚本，给出 ✅/❌ 与实测数字",
    "收口：确认 " + art + " 是工作目录根的唯一交付件，写清「做了什么 / 怎么验的 / 哪里没做完」，然后 complete_slice 交付",
    "补强：把验收脚本里还没覆盖的点补成带数字阈值的断言，打印出来",
  ];
    ...
    body: JSON.stringify({
      ...
      messages: [
        ...
        { role: "user", content:
          ...
          "\n\n【第 1 片请排集成片：「【前 18 分钟必须交付】把所有产出整合进唯一交付件并跑通验收」，其余片排在它后面。】\n\n【片数要求】正好 " + String(target) +
...
export async function seedSlices(
  ...
  if (slices.length === 0) {
      ...
      slices = genericSlices(opts.agents ?? 6, goal);
      source = "fallback";
