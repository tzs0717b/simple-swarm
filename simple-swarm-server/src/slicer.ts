...
export function genericSlices(agents: number): string[] {
  ...
  /* 保底切法：LLM 派工失败时用。
     2026-09-19 复盘（pelican-45b）：旧版 10 片是"目标无关"的粗片 —— 没有唯一交付件、
     没有一条机械阈值、单片过大（"主要运动动画 1"就是一整个子系统）。结果：7/12 片认领后
     卡死，最后交出一个"简化版"骨架，工作区里 7 个 SVG 没人知道哪个是最终件。
     新版把 LLM 硬规则 1 和 3 固化进来：唯一交付件 + 可机械判定的阈值 + 更小的片。 */
  const all = [
    "【前 18 分钟必须交付】先把一个能打开的单文件雏形放到工作目录根，并跑通验收脚本",
    "定下唯一交付件：全队只认工作目录根的那一个成品文件；改主意时先交旧版、再另存 成品_v2，并在交付说明里写清为什么改（不许覆盖旧版）",
    "主体静态造型：主角的轮廓、比例、关键特征（先交静态版，本次不要动画）",
    "次要元素/承载物静态造型：主角所在的环境或道具，与主体的位置关系正确",
    "写验收脚本并跑通，必须打印数字：产物能被解析、结构完整（图形产物就是单根元素且闭合）、能重复渲染 10 次不报错",
    "写机械阈值断言并跑通，必须打印数字：若产物是图形/动画，则相邻帧像素差 ≥ 1%（真的在动）、首帧与末帧差 ≤ 2%（循环闭合）；若不是图形产物，则给出能在命令行跑出数字的验收命令与阈值",
    "主运动①：主体最核心的那个动作（自己判断它是什么）；若它与另一个部件耦合，脚本必须逐帧打印两者的对齐量",
    "主运动②：与动作①耦合的部件，时序必须对齐；给出一条带数字阈值的断言（例如某两点间距 ≤ 5px 或某比值误差 ≤ 1%），逐帧打印",
    "次级细节动画：幅度小、周期与主运动同步的细节",
    "封装为无缝循环：首尾帧一致，把实测首尾差数字写进交付说明",
    "独立复检：别人重新渲染成品并跑验收脚本，给出 ✅/❌ 与实测数字",
  ];
