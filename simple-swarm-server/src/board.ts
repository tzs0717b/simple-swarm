...
export function negotiateKickoffText(swarmId: string, agentCount: number, boardMs: number, goal: string): string {
  ...
  return [
    ...
    "   · 工作区是 git 版本库（重要）：每步结束后**系统都会自动提交一次并署你的名**——所以你怎么写都不会丢，但**改别人写过的文件一定会留下记录**。三条规矩：\n① 想改 / 覆盖别人的文件之前，先在广播里说一声（他的版本号就在你的现场提示里）；\n② 想拿回旧版本：`git show <版本号>:路径 > 路径`，想看差了什么：`git diff <版本号> HEAD -- 路径`；\n③ 交付时把你这版的版本号写进 evidence。\n别为了怕覆盖而发明新文件名——同一个文件的不同版本都在历史里，谁写得对由测试说话。",
